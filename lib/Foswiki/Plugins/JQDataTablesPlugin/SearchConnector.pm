...
sub search {
    ...
    while ( $hits->hasNext ) {
        ...
        if ($formName) {
            ...
            try {
                $formDef =
                  new Foswiki::Form( $this->{session}, $formWeb, $formTopic );
            }
        ...
        foreach my $fieldName ( @{ $params{fields} } ) {
            ...
            if ( !$isEscaped && $propertyName eq 'index' ) {
                ...
                my $topicTitle =
                  $this->getTopicTitle( $Foswiki::cfg{UsersWebName},
                    $author, $topicObj );
                my $html = "<a href='"
                    ...
                    if ( $fieldDef->can("getDisplayValue") ) {
                        ...
                        $html =
                          $fieldDef->renderForDisplay( '$value(display)',
                            $cell, undef, $web, $topic );
                    }
                    $html = Foswiki::Func::expandCommonVariables( $html, $topic,
                        $web );
